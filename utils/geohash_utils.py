...
class QuadTree:
    ...
    def init_ship_dic_row(self, row):
        ...
        if row['MMSI'] not in corresponding_node.ship_dic:
            corresponding_node.ship_dic[row['MMSI']] = []
            corresponding_node.ship_dic[row['MMSI']].append({'time': row['time'], 'lat': row['latitude'], 'lon': row['longitude']})
        corresponding_node.ship_dic[row['MMSI']].append({'time': row['time'], 'lat': row['latitude'], 'lon': row['longitude']})
    ...
    def filter_nodes(self, alpha):
        ...
        for node in leaf_nodes:
            ...
            if len(node.ship_dic) < threshold  or len(node.ship_dic) <= 0:
                node.valid = False
...
def get_adjacency_shape_matrix(convex_hulls, df, adjacency_matrix, length_matrix, width_matrix):
...
def visualize_node_list(df, node_list: [QuadTreeNode], color='red'):
    ...
    m = folium.Map(location=[55, 12], zoom_start=6, tiles=tiles,attr='高德-常规图',)
